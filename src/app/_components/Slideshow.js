          ...
          <div key={index} className={`relative w-full h-screen }`}>
            ...
            {slide.attributes.type === 'video' ? (
              ...
              />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
              <h1 className="text-white text-[58px] font-bold">{slide.attributes.title}</h1>
              <span className='text-white'>{slide.attributes.description}</span>
              <Link href="/categories" className='my-2.5 cursor-pointer'> {slide.attributes.ImageBg === 'dark' ? 
              <span className={`text-black border-white hover:bg-black hover:text-white bg-white opacity-75 font-semibold border-2 px-[40px] py-[14px] `}>Shop New In</span>
            : <span className={`text-white border-black opacity-75 hover:bg-white bg-black hover:text-black font-semibold border-2 px-[40px] py-[14px] `}>Shop New In</span>}  </Link>
            </div></div>
            ) : (
              <div>
